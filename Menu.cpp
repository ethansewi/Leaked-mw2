...
void Visual()
{
	...
	ImGui::Checkbox(xorstr_("Show Health"), &globals::b_health);
...
{
	ImGui::Dummy(ImVec2(0.0f, 3.0f));
	ImGui::Checkbox(xorstr_("Advanced UAV"), &globals::b_UAV);
...
namespace g_menu
{
	void menu()
	{
		...
		if (b_menu_open && screenshot::visuals)
		{
			...
			ImGui::Begin(xorstr_("Kohy's MW2 Chair | VIP"), &b_menu_open, NULL);
			...
			ImGui::SetWindowPos(ImVec2(dwWidth * 2.0f, dwHeight * 0.2f), ImGuiCond_Once);
			{
				ImGui::BeginChild(xorstr_("##TABCHILD"), ImVec2(110, -1), true);
				{
					...
					if (ImGui::Button(xorstr_("ESP/VISUAL"), ImVec2(95, 30))) { i_MenuTab = 0; }
					for (int i = 0; i < 15; i++)
					{
						ImGui::Spacing();
					}
					...
					if (ImGui::Button(xorstr_("Loot Visuals"), ImVec2(95, 30))) { i_MenuTab = 4; }
					for (int i = 0; i < 15; i++)
					{
						ImGui::Spacing();
					}
